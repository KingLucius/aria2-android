...
do
  ...
  ./configure \
    ...
    --with-libssh2 \
    $configure_params \
    CXXFLAGS="-w -pipe -Os -s -flto -fdata-sections -ffunction-sections -Wl,--gc-sections" \
    CFLAGS="-w -pipe -Os -s -flto -fdata-sections -ffunction-sections -Wl,--gc-sections" \
    CPPFLAGS="-fPIE" \
  ...
  make install || exit
  $STRIP $install_dir/bin/aria2c -o $install_dir/libaria2c.so
